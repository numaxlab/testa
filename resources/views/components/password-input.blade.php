@php
    ...
    $inputClass = 'block w-full pe-10' . ($hasError ? ' border-danger' : '');
@endphp
...
<div x-data="{ show: false }">
    ...
    <div class="relative">
        <input
                ...
                {!! $attributes->except('type')->merge(['class' => $inputClass]) !!}
        />
        <button
                type="button"
                @click="show = !show"
                :aria-label="show ? '{{ __('Ocultar contraseña') }}' : '{{ __('Mostrar contraseña') }}'"
                class="absolute inset-y-0 end-0 flex items-center pe-3 cursor-pointer opacity-40 hover:opacity-80"
        >
